...
import { providers, tokenContract, presaleContract } from "../contracts";
...
export default function Provider({ children }) {
  ...
  const checkBalance = async () => {
    try {
      ...
      var ethBalance = fromBigNum(
        await state.provider.getBalance(userAddress),
        18
      );
      
      var tokenBalance = fromBigNum(await tokenContract.balanceOf(userAddress),18);
  ...
  const buy = async (amount) => {
    try {
      if (amount > state.ethBalance) {
        NotificationManager.success("BNB Balance error");
        return;
      }
      ...
      console.log(err);
      NotificationManager.success("Buy error");
    }
  };
